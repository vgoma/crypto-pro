...
exports.getUserCertificates = _afterPluginsLoaded_1._afterPluginsLoaded((resetCache = false, skipCheck = false) => {
    ...
    return eval(_generateCadesFn_1._generateCadesFn(function getUserCertificates() {
        ...
        try {
            void (_generateCadesFn_1.__cadesAsyncToken__ +
                cadesStore.Open(cadesplugin.CADESCOM_CONTAINER_STORE, cadesplugin.CAPICOM_CURRENT_USER_STORE, cadesplugin.CAPICOM_MY_STORE, cadesplugin.CAPICOM_STORE_OPEN_MAXIMUM_ALLOWED));
        }
        catch (error) {
            console.error(error);
